...
from aiogram.types import (
    ...
    ReplyKeyboardMarkup, KeyboardButton, ChatMemberStatus
)
...
CHANNEL_USERNAME = "@jangleaikingf"
DB_PATH = "database.db"
FREE_LIMIT = 2
...
async def cmd_start(message: Message):
    ...
    if not await is_subscribed(u.id):
        await message.answer(
            ...
            "📢 Чтобы начать пользоваться ботом — подпишись на канал:",
...
async def check_sub(cb: CallbackQuery):
    if await is_subscribed(cb.from_user.id):
        await cb.message.edit_text("✅ Подписка подтверждена! Добро пожаловать в Jungle AI.")
        await cb.message.answer("Используй /help для списка команд.", reply_markup=main_keyboard(cb.from_user.id))
    else:
...
async def handle_message(message: Message):
    ...
    if not await is_subscribed(u.id):
        await message.answer("📢 Подпишись на канал, чтобы пользоваться ботом:", reply_markup=sub_keyboard())
        return
